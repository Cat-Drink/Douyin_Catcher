...
本模块定义项目所有自定义异常的根（DouyinCatcherError）、爬虫层异常基类
...
完整异常层次图（含后续里程碑将补充的异常）：
    ...
    └── DouyinCatcherError                    # 项目所有自定义异常的根
        ...
        │   ├── InvalidURLFormatError         # 链接格式无法识别（v0.0.3 补充）
        │   ├── CookieInvalidError            # Cookie 失效或被风控
        │   ├── RateLimitedError              # 触发限流（HTTP 461/412/429）
        │   ├── VideoNotFoundError            # 作品已删除/私密
        │   ├── UserNotFoundError             # 用户主页不存在（v0.0.4 补充）
        │   ├── VerifyRequiredError           # 触发滑块验证（v0.0.4 补充）
